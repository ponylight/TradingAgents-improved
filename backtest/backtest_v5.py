"""Backtest V5 — V3 base strategy + LEVERAGE with 123 Rule confidence booster.

Base strategy: V3 (MACD Reversal with SMA200 bull filter + near-bottom rolling).

Leverage rules:
1. Base leverage: 3x for all entries
2. If 123 Rule confirmed (trendline break + failed retest + pivot break): 5x
3. If price within 20% of 365-day low AND 123 Rule confirmed: 10x (highest conviction)
...
RESULTS_DIR = os.path.join(os.path.dirname(__file__), "results_v5")
...
# V5 constants
NEAR_BOTTOM_THRESHOLD_10X = 0.20  # within 20% of 365-day low for 10x leverage
...
def is_near_365_low_20pct(closes: np.ndarray, current_idx: int) -> bool:
    """Price within 20% of 365-day low (highest conviction zone)."""
    lookback_start = max(0, current_idx - NEAR_BOTTOM_LOOKBACK)
    window = closes[lookback_start:current_idx + 1]
    if len(window) == 0:
        return False
    low_365 = np.min(window)
    return closes[current_idx] <= low_365 * 1.20


def determine_leverage(signal_idx: int, side: str, closes: np.ndarray,
                       highs: np.ndarray, lows: np.ndarray, atr_vals: np.ndarray,
                       swing_highs: list, swing_lows: list) -> tuple[int, bool, bool]:
    """Determine leverage for a trade.

    Returns (leverage, has_123, near_bottom_20).
    """
    has_123 = check_123_rule(signal_idx, side, closes, highs, lows, atr_vals,
                             swing_highs, swing_lows)
    near_bottom_20 = is_near_365_low_20pct(closes, signal_idx)

    if has_123 and near_bottom_20:
        return MAX_LEVERAGE, has_123, near_bottom_20
    elif has_123:
        return LEVERAGE_123, has_123, near_bottom_20
    else:
        return BASE_LEVERAGE, has_123, near_bottom_20
...
class BacktestEngine:
    ...
    def run(self) -> dict:
        print(f"[V5] Running backtest on {len(self.df)} candles...")
        print(f"  Date range: {self.df['timestamp'].iloc[0]} to {self.df['timestamp'].iloc[-1]}")
        print(f"  V5: LEVERAGE (3x base / 5x with 123 Rule / 10x near-bottom + 123 Rule)")
    ...
    def _execute_entry(self, signal: Signal, ts: pd.Timestamp,
        ...
        # Determine leverage
        leverage, has_123, near_bottom_20 = determine_leverage(
        ...
        elif leverage == LEVERAGE_123:
            self.leverage_stats["trades_at_5x"] += 1
    ...
    def _execute_rolling_add(self, signal: Signal, idx: int, ts: pd.Timestamp,
        ...
        # Determine leverage for rolling add
        leverage, _, _ = determine_leverage(
        ...
        for trade in list(self.active_trades):
            ...
            if trade.side == "long":
                ...
                if low <= trade.liquidation_price:
                    ...
                    # At liquidation, lose the remaining margin
    ...
    def _compute_metrics(self) -> dict:
        if not self.closed_trades:
            return {"version": "v5", "error": "No trades executed"}
        ...
        return {
            ...
            "trades_at_5x": self.leverage_stats["trades_at_5x"],
    ...
    def generate_report(self, metrics: dict):
        ...
        self._generate_v3_comparison(metrics)

    def _format_report(self, metrics: dict) -> str:
        sep = "=" * 60
        lines = [
            sep,
            "  BTC/USDT MACD REVERSAL BACKTEST — V5",
            "  V5: LEVERAGE (3x / 5x / 10x) on V3 base strategy",
            sep,
            "",
            ...
            f"  Trades at 5x:    {metrics['trades_at_5x']}  (123 Rule confirmed)",
            f"  Trades at 10x:   {metrics['trades_at_10x']}  (123 Rule + near 365d low)",
            "",
            ...
            "    5x:      if 123 Rule confirmed (trendline break +",
            "             failed retest + pivot break)",
            "    10x:     if 123 Rule confirmed AND price within",
            "             20% of 365-day low (highest conviction)",
            "",
    ...
    def _plot_equity_curve(self):
        ...
        # Try to load V3 equity for overlay
        v3_equity = None
        v3_path = os.path.join(os.path.dirname(__file__), "results_v3")
        # We'll reconstruct V3 comparison in the chart if metrics exist

        fig, (ax1, ax2, ax3) = plt.subplots(3, 1, figsize=(16, 14), height_ratios=[3, 1, 1.5])

        # Equity curve
        ax1.plot(timestamps, equity, color="#E91E63", linewidth=1.4, label="V5 (Leveraged)")
        ax1.axhline(y=self.config.initial_capital, color="gray", linestyle="--", alpha=0.5, label="Initial Capital")

        ax1.set_title("BTC/USDT MACD Reversal V5 — Equity Curve (Leveraged)\n"
                      "(3x base / 5x with 123 Rule / 10x near-bottom + 123 Rule)",
        ...
        if lev_by_trade:
            ...
            for lv in lev_vals:
                if lv >= 10:
                    ...
                    colors.append("#FF9800")  # orange for 5x
                else:
                    colors.append("#2196F3")  # blue for 3x
            ax3.scatter(lev_times, lev_vals, c=colors, s=25, alpha=0.8, zorder=3)
            ax3.axhline(y=3, color="#2196F3", linestyle="--", alpha=0.3, label="3x")
            ax3.axhline(y=5, color="#FF9800", linestyle="--", alpha=0.3, label="5x")
            ax3.axhline(y=10, color="#F44336", linestyle="--", alpha=0.3, label="10x")

        ax3.set_ylabel("Leverage")
        ax3.set_title("Leverage per Trade", fontsize=11)
        ax3.set_ylim(0, 12)
        ax3.set_yticks([0, 3, 5, 10])
    ...
    def _generate_v3_comparison(self, v5_metrics: dict):
        """Generate V5 (leveraged) vs V3 (unleveraged baseline) comparison."""
        ...
        lines = [
            sep,
            "  BACKTEST COMPARISON: V3 (Unleveraged) vs V5 (Leveraged)",
            sep,
            "",
            f"  {'Metric':<25} {'V3 (1x)':>{col_w}} {'V5 (Lev)':>{col_w}}   {'Delta':>15}",
            f"  {'-'*25} {'-'*col_w} {'-'*col_w}   {'-'*15}",
            "",
            f"  {'Total Return %':<25} {v3['total_return_pct']:>{col_w-1}.2f}% {v5['total_return_pct']:>{col_w-1}.2f}%   {fmt_delta(v5['total_return_pct'], v3['total_return_pct'])}",
            f"  {'Final Equity':<25} ${v3['final_equity']:>{col_w-2},.2f} ${v5['final_equity']:>{col_w-2},.2f}   {fmt_mult(v5['final_equity'], v3['final_equity'])}",
            f"  {'Max Drawdown %':<25} {v3['max_drawdown_pct']:>{col_w-1}.2f}% {v5['max_drawdown_pct']:>{col_w-1}.2f}%   {fmt_delta(v5['max_drawdown_pct'], v3['max_drawdown_pct'], False)}",
            f"  {'Sharpe Ratio':<25} {v3['sharpe_ratio']:>{col_w}.2f} {v5['sharpe_ratio']:>{col_w}.2f}   {fmt_delta(v5['sharpe_ratio'], v3['sharpe_ratio'])}",
            f"  {'Win Rate %':<25} {v3['win_rate_pct']:>{col_w-1}.2f}% {v5['win_rate_pct']:>{col_w-1}.2f}%   {fmt_delta(v5['win_rate_pct'], v3['win_rate_pct'])}",
            f"  {'Profit Factor':<25} {v3['profit_factor']:>{col_w}.2f} {v5['profit_factor']:>{col_w}.2f}   {fmt_delta(v5['profit_factor'], v3['profit_factor'])}",
            "",
            f"  {'Total Trades':<25} {v3['total_trades']:>{col_w}} {v5['total_trades']:>{col_w}}",
            f"  {'  Primary':<25} {v3['primary_trades']:>{col_w}} {v5['primary_trades']:>{col_w}}",
            f"  {'  Rolling Adds':<25} {v3['rolling_adds']:>{col_w}} {v5['rolling_adds']:>{col_w}}",
            f"  {'  Long Trades':<25} {v3['long_trades']:>{col_w}} {v5['long_trades']:>{col_w}}",
            f"  {'  Short Trades':<25} {v3['short_trades']:>{col_w}} {v5['short_trades']:>{col_w}}",
            "",
            f"  {'Long PnL':<25} ${v3['long_pnl']:>{col_w-2},.2f} ${v5['long_pnl']:>{col_w-2},.2f}   {fmt_mult(v5['long_pnl'], v3['long_pnl'])}",
            f"  {'Short PnL':<25} ${v3['short_pnl']:>{col_w-2},.2f} ${v5['short_pnl']:>{col_w-2},.2f}   {fmt_mult(v5['short_pnl'], v3['short_pnl'])}",
            "",
            f"  {'Avg Win':<25} ${v3['avg_win']:>{col_w-2},.2f} ${v5['avg_win']:>{col_w-2},.2f}   {fmt_mult(v5['avg_win'], v3['avg_win'])}",
            f"  {'Avg Loss':<25} ${v3['avg_loss']:>{col_w-2},.2f} ${v5['avg_loss']:>{col_w-2},.2f}   {fmt_mult(abs(v5['avg_loss']), abs(v3['avg_loss']))}",
            f"  {'Largest Win':<25} ${v3['largest_win']:>{col_w-2},.2f} ${v5['largest_win']:>{col_w-2},.2f}   {fmt_mult(v5['largest_win'], v3['largest_win'])}",
            f"  {'Largest Loss':<25} ${v3['largest_loss']:>{col_w-2},.2f} ${v5['largest_loss']:>{col_w-2},.2f}   {fmt_mult(abs(v5['largest_loss']), abs(v3['largest_loss']))}",
            f"  {'Avg Duration (days)':<25} {v3['avg_trade_duration_days']:>{col_w}.1f} {v5['avg_trade_duration_days']:>{col_w}.1f}",
            "",
            sep,
            "  V5 LEVERAGE BREAKDOWN",
            sep,
            "",
            f"  Average Leverage:     {v5['avg_leverage']:.1f}x",
            f"  Trades at 3x (base):  {v5['trades_at_3x']}",
            f"  Trades at 5x (123):   {v5['trades_at_5x']}",
            f"  Trades at 10x (max):  {v5['trades_at_10x']}",
            f"  Liquidations:         {v5['liquidations']}",
            f"  Liquidation Losses:   ${v5['liquidation_loss']:,.2f}",
            "",
            sep,
            "  NOTES",
            sep,
            "",
            "  V3 (unleveraged baseline):",
            "    - Spot trading (1x), no leverage",
            "    - Same signal generation, same filters",
            "",
            "  V5 (leveraged):",
            "    - Same V3 signals, but position sizes multiplied by leverage",
            "    - 3x base leverage on all entries",
            "    - 5x when 123 Rule pattern is confirmed",
            "    - 10x when 123 Rule + price within 20% of 365-day low",
            "    - Liquidation at margin exhaustion (notional / leverage)",
            "    - Commission on full notional exposure",
            "",
            sep,
        ]

        comparison = "\n".join(lines)
        comp_path = os.path.join(RESULTS_DIR, "comparison_v3_vs_v5.txt")
