"""Backtest V8 — V6 base strategy + Graduated Leverage Bottom-Fishing.
...
MODE 2 (Graduated Leverage Bottom-Fishing):
  Triggers when ALL conditions met (same as V7):
    - Price within 30% of 365-day low
    - Price below 200 SMA
    - RSI(14) weekly < 30

  Graduated leverage (NEW in V8 — replaces flat 50x from V7):
    - Attempt 1:  5x leverage, 2% margin (stop ~20% away — survives noise)
    - Attempt 2: 10x leverage, 2% margin (stop ~10% away)
    - Attempt 3: 15x leverage, 2% margin (stop ~6.7% away)
    - Attempt 4: 25x leverage, 2% margin (stop ~4% away)
    - Attempt 5: 50x leverage, 2% margin (stop ~2% away — max conviction)

  Each attempt uses 2% of current capital. Max 5 attempts = 10% total risk.
  30-day cooldown after 5 fails.

  Profit mode (same as V7):
    At +5% gain: trail to breakeven, start rolling with floating profits,
    pyramid adds, 2x ATR trailing stop. No fixed TP.
...
RESULTS_DIR = os.path.join(os.path.dirname(__file__), "results_v8")
...
MM_GRADUATED_LEVERAGE = [5, 10, 15, 25, 50]  # leverage per attempt
...
class BacktestEngine:
    def __init__(self, df: pd.DataFrame, config: BacktestConfig | None = None):
        ...
        self.mm_attempt_count = 0       # 0-indexed: current attempt number
    ...
    def _get_current_mm_leverage(self) -> float:
        """Get leverage for current attempt (graduated: 5x→10x→15x→25x→50x)."""
    ...
    def _enter_mm_trade(self, idx: int, price: float, ts: pd.Timestamp,
        ...
        """Enter a Mode 2 bottom-fish trade with graduated leverage."""
    ...
    def _handle_mm_stop(self, idx: int, close: float, ts: pd.Timestamp):
        """Handle a Mode 2 stop-out in Phase A."""
        ...
        if self.mm_attempt_count >= MM_MAX_ATTEMPTS:
            ...
            # Re-enter at current close with next leverage tier
            self.mm_base_trade = None
    ...
    def _add_mm_pyramid(self, idx: int, price: float, ts: pd.Timestamp):
        ...
        # Pyramids use the base trade's leverage
        leverage = self.mm_base_trade.leverage if self.mm_base_trade else MM_GRADUATED_LEVERAGE[-1]
        notional = pyramid_margin * leverage
        size_btc = notional / price
        commission = notional * self.config.commission_rate
        self.capital -= commission
        ...
        trade = MMTrade(
            entry_price=price, entry_idx=idx, entry_time=ts,
            size=size_btc, margin=pyramid_margin, leverage=leverage,
        ...
        print(f"    [MM] Pyramid #{self.mm_pyramid_count} at ${price:,.0f} | "
              f"{leverage}x | margin=${pyramid_margin:,.0f} (from floating) | size={size_btc:.4f} BTC")
    ...
    def _compute_metrics(self) -> dict:
        ...
        if not all_mode1 and not all_mode2:
            return {"version": "v8", "error": "No trades executed"}
    ...
    def _format_report(self, m: dict) -> str:
        ...
        lines = [
            sep,
            "  BTC/USDT BACKTEST — V8",
            "  Mode 1: V6 MACD Reversal (3x/10x)",
            "  Mode 2: Graduated Bottom-Fishing (5x→10x→15x→25x→50x)",
            sep,
            "",
        ...
        lines += [
            "",
            "  Mode 2 Rules (V8 — graduated from V7):",
            "    Entry: price within 30% of 365d low + below SMA200 + wRSI<30",
            "    Graduated leverage: 5x → 10x → 15x → 25x → 50x",
    ...
    def _plot_equity_curve(self):
        ...
        # ── Equity curve ──
        ax1.plot(timestamps, equity, color="#00BCD4", linewidth=1.4, label="V8 Equity (Mode 1 + Mode 2 Graduated)")
        ...
        # Mode 2 trade markers (diamonds) with size proportional to leverage
        for trade in self.mm_closed_trades:
            ...
            if trade.entry_time in timestamps.values:
                eidx = timestamps[timestamps == trade.entry_time].index
                if len(eidx) > 0:
                    ms = 4 + trade.leverage / 5  # scale: 5→5, 10→6, 15→7, 25→9, 50→14
                    ax1.plot(trade.entry_time, equity[eidx[0]],
                             marker="D", color=mc, markersize=ms, alpha=0.8)

        ax1.set_title("BTC/USDT V8 — Equity Curve\n"
                       "(Mode 1: V6 MACD 3x/10x | Mode 2: Graduated Bottom-Fish 5x→50x)",
        ...
        # ── Leverage timeline (Mode 1 scatter + Mode 2 spans) ──
        mm_entries = []
        ...
        # Mode 2 leverage dots
        for trade in self.mm_closed_trades:
            if not trade.is_pyramid:
        ...
        ax3.set_title("Mode 1 Leverage + Mode 2 Graduated Activity", fontsize=11)
        ax3.set_ylim(0, 55)
    ...
    def _generate_comparison(self, v8_metrics: dict):
        """Generate V6 vs V7 vs V8 comparison."""
        v6_path = os.path.join(os.path.dirname(__file__), "results_v6", "metrics.json")
        v7_path = os.path.join(os.path.dirname(__file__), "results_v7", "metrics.json")
        v6 = None
        v7 = None
        ...
        if v7 is not None:
            lines = [
                sep,
                "  BACKTEST COMPARISON: V6 vs V7 (50x flat) vs V8 (graduated 5x→50x)",
                sep,
                "",
                f"  {'Metric':<28} {'V6 (MACD only)':>{cw}} {'V7 (V6+50x MM)':>{cw}} {'V8 (V6+grad MM)':>{cw}}   {'V8 vs V6':>12}",
                f"  {'-'*28} {'-'*cw} {'-'*cw} {'-'*cw}   {'-'*12}",
                "",
                f"  {'Total Return %':<28} {v6['total_return_pct']:>{cw-1}.2f}% {v7['total_return_pct']:>{cw-1}.2f}% {v8['total_return_pct']:>{cw-1}.2f}%   {fmt_delta(v8['total_return_pct'], v6['total_return_pct'])}",
                f"  {'Final Equity':<28} ${v6['final_equity']:>{cw-2},.2f} ${v7['final_equity']:>{cw-2},.2f} ${v8['final_equity']:>{cw-2},.2f}   {fmt_mult(v8['final_equity'], v6['final_equity'])}",
                f"  {'Max Drawdown %':<28} {v6['max_drawdown_pct']:>{cw-1}.2f}% {v7['max_drawdown_pct']:>{cw-1}.2f}% {v8['max_drawdown_pct']:>{cw-1}.2f}%   {fmt_delta(v8['max_drawdown_pct'], v6['max_drawdown_pct'], False)}",
                f"  {'Sharpe Ratio':<28} {v6['sharpe_ratio']:>{cw}.2f} {v7['sharpe_ratio']:>{cw}.2f} {v8['sharpe_ratio']:>{cw}.2f}   {fmt_delta(v8['sharpe_ratio'], v6['sharpe_ratio'])}",
                f"  {'Win Rate %':<28} {v6['win_rate_pct']:>{cw-1}.2f}% {v7['win_rate_pct']:>{cw-1}.2f}% {v8['win_rate_pct']:>{cw-1}.2f}%   {fmt_delta(v8['win_rate_pct'], v6['win_rate_pct'])}",
                f"  {'Profit Factor':<28} {v6['profit_factor']:>{cw}.2f} {v7['profit_factor']:>{cw}.2f} {v8['profit_factor']:>{cw}.2f}   {fmt_delta(v8['profit_factor'], v6['profit_factor'])}",
                "",
                f"  {'Total Trades':<28} {v6['total_trades']:>{cw}} {v7['total_trades']:>{cw}} {v8['total_trades']:>{cw}}",
            ]

            # V7 vs V8 Mode 2 comparison
            lines += [
                "",
                sep,
                "  MODE 2 COMPARISON: V7 (flat 50x) vs V8 (graduated)",
                sep,
                "",
                f"  {'':28} {'V7 (50x flat)':>{cw}} {'V8 (graduated)':>{cw}}",
                f"  {'':28} {'-'*cw} {'-'*cw}",
                f"  {'M2 Total Trades':<28} {v7['m2_total_trades']:>{cw}} {v8['m2_total_trades']:>{cw}}",
                f"  {'M2 Total PnL':<28} ${v7['m2_total_pnl']:>{cw-2},.2f} ${v8['m2_total_pnl']:>{cw-2},.2f}",
                f"  {'M2 Win Rate %':<28} {v7['m2_win_rate_pct']:>{cw-1}.2f}% {v8['m2_win_rate_pct']:>{cw-1}.2f}%",
                f"  {'M2 Entry Attempts':<28} {v7['m2_total_entries']:>{cw}} {v8['m2_total_entries']:>{cw}}",
                f"  {'M2 Phase A Stops':<28} {v7['m2_stops_phase_a']:>{cw}} {v8['m2_stops_phase_a']:>{cw}}",
                f"  {'M2 Successful Rides':<28} {v7['m2_successful_rides']:>{cw}} {v8['m2_successful_rides']:>{cw}}",
                f"  {'M2 Cooldowns':<28} {v7['m2_cooldowns_triggered']:>{cw}} {v8['m2_cooldowns_triggered']:>{cw}}",
                f"  {'M2 Pyramid Adds':<28} {v7['m2_total_pyramid_adds']:>{cw}} {v8['m2_total_pyramid_adds']:>{cw}}",
                f"  {'M2 Best Ride PnL':<28} ${v7['m2_best_ride_pnl']:>{cw-2},.2f} ${v8['m2_best_ride_pnl']:>{cw-2},.2f}",
                f"  {'M2 Worst Stop PnL':<28} ${v7['m2_worst_stop_pnl']:>{cw-2},.2f} ${v8['m2_worst_stop_pnl']:>{cw-2},.2f}",
                f"  {'M2 Avg Leverage':<28} {'50.0':>{cw}} {v8['m2_avg_leverage']:>{cw}.1f}",
            ]
        else:
            lines = [
                sep,
                "  BACKTEST COMPARISON: V6 vs V8 (graduated 5x→50x)",
                sep,
                "",
                f"  {'Metric':<28} {'V6 (MACD only)':>{cw}} {'V8 (V6+grad MM)':>{cw}}   {'Delta':>15}",
                f"  {'-'*28} {'-'*cw} {'-'*cw}   {'-'*15}",
                "",
                f"  {'Total Return %':<28} {v6['total_return_pct']:>{cw-1}.2f}% {v8['total_return_pct']:>{cw-1}.2f}%   {fmt_delta(v8['total_return_pct'], v6['total_return_pct'])}",
                f"  {'Final Equity':<28} ${v6['final_equity']:>{cw-2},.2f} ${v8['final_equity']:>{cw-2},.2f}   {fmt_mult(v8['final_equity'], v6['final_equity'])}",
                f"  {'Max Drawdown %':<28} {v6['max_drawdown_pct']:>{cw-1}.2f}% {v8['max_drawdown_pct']:>{cw-1}.2f}%   {fmt_delta(v8['max_drawdown_pct'], v6['max_drawdown_pct'], False)}",
                f"  {'Sharpe Ratio':<28} {v6['sharpe_ratio']:>{cw}.2f} {v8['sharpe_ratio']:>{cw}.2f}   {fmt_delta(v8['sharpe_ratio'], v6['sharpe_ratio'])}",
                f"  {'Win Rate %':<28} {v6['win_rate_pct']:>{cw-1}.2f}% {v8['win_rate_pct']:>{cw-1}.2f}%   {fmt_delta(v8['win_rate_pct'], v6['win_rate_pct'])}",
                f"  {'Profit Factor':<28} {v6['profit_factor']:>{cw}.2f} {v8['profit_factor']:>{cw}.2f}   {fmt_delta(v8['profit_factor'], v6['profit_factor'])}",
                "",
                f"  {'Total Trades':<28} {v6['total_trades']:>{cw}} {v8['total_trades']:>{cw}}",
            ]

        # V8 graduated leverage breakdown
        lines += [
            "",
            sep,
            "  V8 GRADUATED LEVERAGE BREAKDOWN",
        ...
        for lev in MM_GRADUATED_LEVERAGE:
            entries = v8['m2_entries_per_leverage'].get(str(lev), 0)
            stops = v8['m2_stops_per_leverage'].get(str(lev), 0)
            wins = v8['m2_wins_per_leverage'].get(str(lev), 0)
            stop_dist = (1/lev) * 100
        ...
        lines += [
            ...
            "",
            "  V7 = V6 + flat 50x bottom-fishing:",
            "    - Mode 2 always 50x (stop ~2% away)",
            "    - High risk of liquidation in volatile markets",
            "    - All-or-nothing conviction",
            "",
            "  V8 = V6 + GRADUATED bottom-fishing:",
            "    - Mode 2 starts at 5x (stop ~20% away — survives noise)",
            "    - Escalates: 5x → 10x → 15x → 25x → 50x",
            "    - Earlier attempts survive more volatility",
            "    - Later attempts increase conviction after confirms",
            "    - Same 2% capital per attempt, 10% total risk",
            "",
            sep,
        ]

        comparison = "\n".join(lines)
        comp_path = os.path.join(RESULTS_DIR, "comparison_v6_v7_v8.txt")
